...
### 6.3 通用首部字段
    Cache-Control: no-cache, no-store, max-age=3600, no-transform, cache-extentsion,
...
### 6.5 实体首部字段
    Allow / Content-Encoding / Content-Length / Content-Language / Content-Location / Content-MD5 / Content-Range / Content-Type / Expires / Last-Modified 
### 6.8 其他首部字段
...
### 9.3 使用浏览器进行全双工的WebSocket
### 9.4 HTTP/2.0
    SPDY的多路复用和流量控制、HTTP Speed+Mobility的TLS义务化／协商／客户端拉拽／服务器推送／WebSocket
